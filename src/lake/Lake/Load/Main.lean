      ...
      let depPkgs ← deps.mapM fun dep => fetchOrCreate dep.name do
        if let some entry := pkgEntries.find? dep.name then
          ...
          error <|
            ...
            s!"check with the developer of {pkg.name} to see if they forgot to update their manifest file," ++
            s!"or use `lake update` to generate a new, complete file (warning: this will update ALL workspace dependencies)"

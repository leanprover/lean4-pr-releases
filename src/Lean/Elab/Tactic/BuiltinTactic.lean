...
private def getCaseGoals (tag : TSyntax ``binderIdent) : TacticM (MVarId × List MVarId) := do
  ...
  let g ← if let `(binderIdent| $tag:ident) := tag then
    let tag := tag.getId
    let some g ← findTag? gs tag | notFound tag gs
    pure g
  else
    getMainGoal
  return (g, gs.erase g)
  ...
  where
    ...
    notFound (tag : Name) (available : List MVarId) := do
      let firstLine := m!"Case tag {tag} not found."
      ...
      | [availableName] => throwError "{firstLine}\n\nThe only available case tag is {availableName}."
      | availableNames =>
        throwError "Case tag {tag} not found.\n\nAvailable tags:{commaList <| availableNames.map toMessageData}"

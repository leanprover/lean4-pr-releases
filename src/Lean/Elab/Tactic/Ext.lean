...
def applyExtTheorem (goal : MVarId) : MetaM (List MVarId) := goal.withContext do
...
elab "apply_ext_theorem" : tactic => liftMetaTactic applyExtTheorem
...
def withExt1 [Monad m] [MonadLiftT TermElabM m] (g : MVarId) (pats : List (TSyntax `rcasesPat))
    (k : MVarId → List (TSyntax `rcasesPat) → m Nat) : m Nat := do
  let mut n := 0
  for g in ← (applyExtTheorem g : TermElabM _) do
